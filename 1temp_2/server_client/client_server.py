...
from pymodbus.datastore import ModbusSequentialDataBlock, ModbusSlaveContext, ModbusServerContext # Modbus 데이터 저장소 관련 클래스
...
# 로깅 설정
logging.basicConfig(format="%(asctime)s %(levelname)-8s %(message)s", level=logging.INFO) # 로그 메시지 형식 지정, 로그 레벨을 INFO 이상으로 설정
...
class DBDataBlock(ModbusSequentialDataBlock):
    ...
    def __init__(self, cur: sqlite3.Cursor, table: str):
        super().__init__(0, [0]*10) # 부모 클래스(ModbusSequentialDataBlock) 초기화. 주소 0부터 100개의 레지스터, 초기값 0
    ...
    def getValues(self, addr: int, cnt: int = 1) -> List[int]:
        ...
        for i in range(cnt): # 요청된 개수(cnt)만큼 반복
            ...
            self.cur.execute(f"SELECT value FROM {self.table} WHERE address=?", (to_hex(addr + i),)) # 주소는 16진수 문자열로 변환하여 사용
    ...
    def setValues(self, addr: int, vals: Sequence[int]):
        for i, v in enumerate(vals): # 쓰려는 값들(vals)에 대해 반복
            ...
            self.cur.execute(f"UPDATE {self.table} SET value=? WHERE address=?", (to_hex(v), to_hex(addr + i))) # 값과 주소를 16진수 문자열로 변환
...
class Device(QObject): # PySide2의 QObject를 상속받아 시그널/슬롯 기능 사용
    ...
    def __init__(self, unit: int, client: AsyncModbusTcpClient, parent: QObject):
    ...
    async def _loop(self):
        ...
        while True: # 무한 루프
            try:
                ...
                res = await self.cli.read_holding_registers(address=5, count=5, slave=self.unit)  # address: 시작 주소, count: 읽을 개수, slave: 대상 unit ID
                if res.isError(): raise Exception(res) # 응답에 에러가 있으면 예외 발생
                regs = res.registers # 응답에서 레지스터 값 리스트 추출
                ...
                self.logSignal.emit(str(self.unit), "[READ] -> %s" % regs) # logSignal 시그널 발생
            except Exception as e: # READ 동작 중 예외 발생 시
                self.logSignal.emit(str(self.unit), "[ERR] READ: %s" % e) # 에러 로그 시그널 발생
    ...
    async def write_single(self, addr: int, val: int):
        try:
            ...
            self.logSignal.emit(str(self.unit), "[WRITE1] addr=%d val=%s" % (addr, val)) # 성공 로그 시그널 발생
        except Exception as e: # 쓰기 동작 중 예외 발생 시
            self.logSignal.emit(str(self.unit), "[ERR] WRITE1: %s" % e) # 에러 로그 시그널 발생

    # QML에서 멀티 라이트 요청할 때 호출되는 비동기 메소드
    async def write_multi(self, addr: int, vals: List[int]):
        try:
            ...
            self.logSignal.emit(str(self.unit), "[WRITE_N] vals=%s" % vals) # 성공 로그 시그널 발생
        except Exception as e: # 쓰기 동작 중 예외 발생 시
...
class Backend(QObject):
    ...
    async def init(self, cur: sqlite3.Cursor):
        ...
        blocks = {sid: DBDataBlock(cur, f"holding_registers_{sid}") for sid in (1, 2, 3)}
        # 각 슬레이브 ID에 대해 ModbusSlaveContext 생성 (데이터 블록 연결)
        slaves_context = {sid: ModbusSlaveContext(hr=block, zero_mode=True) for sid, block in blocks.items()} # hr: holding registers
        ...
        asyncio.create_task(StartAsyncTcpServer(context, address=("localhost", SLAVE_PORT)))
        ...
        for unit_id in (1, 2, 3): # unit ID 1, 2, 3에 대해 반복
            ...
            device = Device(unit_id, client, parent=self)
            # Device 객체의 시그널을 Backend 객체의 해당 시그널로 연결
            # 이렇게 하면 Device에서 발생한 시그널이 Backend를 거쳐 QML로 전달될 수 있음
            device.readReady.connect(self.readData)  # Device의 readReady -> Backend의 readData
            device.logSignal.connect(self.logSignal)  # Device의 logSignal -> Backend의 logSignal
            self.devices[unit_id] = device # 생성된 Device 객체를 딕셔너리에 저장
    ...
    def writeMulti(self, unit_str: str, vals_qml: list):
        ...
        asyncio.create_task(self.devices[u].write_multi(0, ints))
